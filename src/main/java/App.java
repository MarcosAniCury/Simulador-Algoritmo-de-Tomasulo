            ...
            for (ReservationStation reservationStation : ReservationStationController.allReservationsArea.values()) {
                ...
                for (int i = 0; i < reservationStationInstructions.length; i++) {
                    ...
                    if (reservationStationInstructions[i].getRegisterOne().getValue() != -1) {
                        String firstRegisterInstructionName = reservationStationInstructions[i].getRegisterOne()
                                .getBufferInstruction().getInstruction().getInstruction();
                        if (Arrays.stream(Instructions.LOAD_STORE)
                                .anyMatch(item -> item.equals(firstRegisterInstructionName))) {
                            addressFirstRegister = "Mem[Regs["
                                    + reservationStationInstructions[i].getRegisterOne().getName() + "]]";
                        } else {
                            addressFirstRegister = "Regs["
                                    + reservationStationInstructions[i].getRegisterOne().getName() + ']';
                        }
                    } else {
                        valueFirstRegister = "#" + reservationStationInstructions[i].getRegisterOne()
                                .getBufferInstruction().getRegisterDestination().getName();
                    }
                    String addressSecondRegister = "";
                    String valueSecondRegister = "";
                    if (reservationStationInstructions[i].getRegisterTwo().getValue() != -1) {
                        String secondRegisterInstructionName = reservationStationInstructions[i].getRegisterTwo()
                                .getBufferInstruction().getInstruction().getInstruction();
                        if (Arrays.stream(Instructions.LOAD_STORE)
                                .anyMatch(item -> item.equals(secondRegisterInstructionName))) {
                            addressSecondRegister = "Mem[Regs["
                                    + reservationStationInstructions[i].getRegisterTwo().getName() + "]]";
                        } else {
                            addressSecondRegister = "Regs["
                                    + reservationStationInstructions[i].getRegisterTwo().getName() + ']';
                        }
                    } else {
                        valueSecondRegister = "#" + reservationStationInstructions[i].getRegisterTwo()
                                .getBufferInstruction().getRegisterDestination().getName();
                    }
